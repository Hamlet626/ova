...
export const getUserBasicFbData_client=cache(async(role:RoleNum,uid:string)=>{
    const r = await getDoc(doc(getFirestore(app),UserDoc(role,uid)));
        return r.data() as BasicInfoDoc;
});

export const getUserAgcFbData_client=cache(async(role:RoleNum,uid:string,agcid:string)=>{
...
export const getUserAlgoData_client=cache(async(role:RoleNum,uid:string)=>{
